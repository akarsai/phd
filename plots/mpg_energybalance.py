...
from examples.quasilinear_wave import QuasilinearWave, QuasilinearWaveReducedOrder
...
def energybalance(
    ...
    if kind == 'toda':
        ebm_fom = Toda()
        num_proj_nodes_list = [4,4,4] # hardcoded for 1,2,3
    elif kind == 'rigid_body':
        ebm_fom = RigidBody()
        num_proj_nodes_list = [k for k in degrees]
    elif kind == 'acdc':
        ebm_fom = ACDC()
        num_proj_nodes_list = [2*k for k in degrees]
    elif kind == 'quasilinear_wave':
        ebm_fom = QuasilinearWave(**ebm_kwargs)
    ...
    for order in orderlist:
        ...
        if order == 'rom':
            if kind == 'cahn_hilliard':
                ...
                ebm_rom = QuasilinearWaveReducedOrder(picklepath=picklepath, **ebm_kwargs) # picklepath to speed up computation
            ebm = ebm_rom
