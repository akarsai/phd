...
#
# this file tests the implementation of the proposed discrete
# gradient method for QSR dissipative systems
#
#
...
if __name__ == '__main__':
    ...
    # time discretization
    from main.time_discretization import implicit_midpoint, QSR_discrete_gradient

    # visualization
    import matplotlib.pyplot as plt
    from helpers.visualization import QSR_discrete_gradient_powerbalance
    ...
    # examples
    from examples.lti import LTI_withOptimalControl, PI_Controller
    from examples.pendulum import Pendulum
    from examples.hill_moylan import HillMoylan

    # example controls
    sine_control = jax.vmap(lambda t: jnp.array([jnp.sin(2*t)]))
    # fast_sine_control = jax.vmap(lambda t: jnp.array([jnp.sin(1000*t)]))
    zero_control = jax.vmap(lambda t: jnp.zeros((1,)))
    speeding_up_sine_control = jax.vmap(lambda t: jnp.array([jnp.sin(t**2 / 4)]))
    ...
    # plants
    lti = LTI_withOptimalControl()
    pi_controller = PI_Controller()
    pendulum = Pendulum()
    # van_der_pol = VanDerPol_withOptimalControl()
    hill_moylan = HillMoylan()

    # general settings
    T = 10.0 # final time, default 10.0
    base_Delta_t = 1e-3 # smallest tested delta t, default 1e-3
    num_Delta_t_steps = 9 # maximum tested delta t given by 2**steps * smallest delta t, default 9
    Delta_t_array = jnp.array([2**i * base_Delta_t for i in range(num_Delta_t_steps)])
    ref_order_smaller = 3 # by which order of magnitude should the reference solution be smaller than the smallest tested Delta t?, default 3
    ...
    ax_balances.set_ylabel(r'$\errorenergyabs$\,')
    ax_balances.set_ylim([.5e-18, .5e-3])

    # which systems to test
    tested_systems = ['pendulum', 'lti', 'pi_controller', 'hill_moylan']
    ...
    for tested_system in tested_systems:
        ...
        # set savepath
        SAVEPATH = './results/figures/dg/qsr_dg_all'
        

        if tested_system == 'lti':
            system = lti
            control = speeding_up_sine_control
            ell = lambda z: 1/jnp.sqrt(2) * system.h_orig(z)
            W = lambda z: jnp.zeros((system.ncontrol, system.ncontrol))
            QSR = (1/2 * jnp.eye(system.ncontrol), 1/2 * jnp.eye(system.ncontrol), jnp.zeros((system.ncontrol, system.ncontrol)))
            system_label = r'Example~\ref{ex:qsr-hjb}'
        elif tested_system == 'pendulum':
            system = pendulum
            control = sine_control
            ell = lambda z: jnp.zeros((system.ncontrol,))
            W = lambda z: jnp.zeros((system.ncontrol, system.ncontrol))
            QSR = (- pendulum.friction_coefficient * jnp.eye(system.ncontrol), 1/2 * jnp.eye(system.ncontrol), jnp.zeros((system.ncontrol, system.ncontrol)))
            ...
            system = hill_moylan
            control = bump_control
            ell = lambda z: jnp.sqrt(hill_moylan.alpha) * z/jnp.sqrt(1 + z**4)
            W = lambda z: jnp.zeros((system.ncontrol, system.ncontrol))
            QSR = (- jnp.eye(system.ncontrol), jnp.zeros((system.ncontrol, system.ncontrol)), hill_moylan.lam**2 * jnp.eye(system.ncontrol))
            system_label = r'Example~\ref{ex:qsr-hill-moylan}'
        else:
        ...
        for k in range(num_Delta_t_steps):
            ...
            ham_zz = hamiltonian_vmap(zz)

            ell_mid_zz = jax.vmap(ell)(1/2 * (zz[:-1] + zz[1:]))
        ...
        # compute powerbalance and supply balance errors
        _, powerbalance_errors, supplybalance_errors = QSR_discrete_gradient_powerbalance(
            system.f,
            system.g,
            system.k,
            ham_eta,
            ...
            zz_ham,
            uu_powerbalance,
            # title = f'{tested_system}',
            # savepath=f'{SAVEPATH}_balances',
            )

        # plot balances
        ax_balances.semilogy(tt_powerbalance[:-1], supplybalance_errors, label=system_label, linewidth=3.0, alpha=0.6)

    ### save and show plots
    # SAVEPATH = '../results/all'
    print('\n\n')
    ...
    ax_convergence.loglog(Delta_t_array, secondorder, label='$\\tau^2$', linewidth=3.0, linestyle='--', color='black', zorder=0, alpha=0.4)
    # beautify
